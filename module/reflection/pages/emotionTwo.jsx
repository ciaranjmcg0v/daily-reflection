...
function EmotionPageTwo() {
    const { name, emojiAfter, setEmojiAfter, setEmojiAfterType, nextStep, emojiAfterType } =
    useReflectionStore();
  ...
  return (
    <div className="flex flex-col w-screen h-screen items-center justify-center">
      ...
      <div className="flex flex-col items-center mb-6">
        <div className="text-4xl mb-2">{emojiAfter}</div>
        <div className="text-lg text-gray-600">
          {emojiAfterType || "No emotion selected"}
        </div>
        <button
          onClick={() => setIsDialogOpen(true)}
          className="mt-2 text-blue-600 hover:underline"
        >
          {emojiAfterType ? "Change Emotion" : "Choose Emotion"}
        </button>
      </div>
